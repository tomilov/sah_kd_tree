...
std::vector<uint8_t> Renderer::loadPipelineCache(std::string_view pipelineCacheName) const
{
    ...
    for (const auto & cacheLocation : cacheLocations) {
        ...
        if (!cacheDir.exists()) {
            qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of pipeline cache directory '%2' does not exists").arg(cacheDir.path(), cacheFileName);
            continue;
        }
        QFileInfo cacheFileInfo{cacheDir, cacheFileName};
        if (!cacheFileInfo.exists()) {
            qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of pipeline cache file '%2' does not exists").arg(cacheFileInfo.path(), cacheFileName);
            continue;
        }
        QFile cacheFile{cacheFileInfo.filePath()};
        if (!cacheFile.open(QIODeviceBase::OpenModeFlag::ReadOnly)) {
            qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of pipeline cache file '%2' cannot be opened to read").arg(cacheFile.fileName(), cacheFileName);
            continue;
        }
        ...
        if (bytesRead < 0) {
            qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Failed to read pipeline cache data '%1' from file '%2': %3").arg(cacheFileName, cacheFile.fileName(), cacheFile.errorString());
            continue;
        }
        if (bytesRead != cacheFileSize) {
            qCWarning(viewerRendererCategory).noquote()
        ...
        qCInfo(viewerRendererCategory).noquote() << QStringLiteral("Pipeline cache data '%1' successfully read from file '%2'").arg(cacheFileName, cacheFile.fileName());
        return cacheData;
    }
    qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Failed to load pipeline cache from '%1' location").arg(cacheFileName);
    return {};
}
...
bool Renderer::savePipelineCache(const std::vector<uint8_t> & data, std::string_view pipelineCacheName) const
{
    ...
    if (!cacheFile.open(QIODeviceBase::OpenModeFlag::WriteOnly)) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of pipeline cache file '%2' cannot be opened to wrie").arg(cacheFile.fileName(), cacheFileName);
        return false;
    }
    ...
    if (bytesWritten < 0) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Failed to write pipeline cache data '%1' to file '%2': %3").arg(cacheFileName, cacheFile.fileName(), cacheFile.errorString());
        return false;
    }
    ...
    if (!cacheFile.commit()) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Failed to commit writing of pipeline cache data '%1' to file '%2'").arg(cacheFileName, cacheFile.fileName(), cacheFile.errorString());
        return false;
    }
    qCInfo(viewerRendererCategory).noquote() << QStringLiteral("Pipeline cache data '%1' successfully written to file '%2'").arg(cacheFileName, cacheFile.fileName());
    return true;
}
...
std::vector<uint32_t> Renderer::loadShader(std::string_view shaderName) const
{
    ...
    if (!shaderDir.exists()) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of shader directory '%2' does not exists").arg(shaderDir.path(), shaderFileName);
        return {};
    }
    QFileInfo shaderFileInfo{shaderDir, shaderFileName};
    if (!shaderFileInfo.exists()) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of shader file '%2' does not exists").arg(shaderFileInfo.path(), shaderFileName);
        return {};
    }
    QFile shaderFile{shaderFileInfo.filePath()};
    if (!shaderFile.open(QIODeviceBase::OpenModeFlag::ReadOnly)) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Possible location '%1' of shader file '%2' cannot be opened to read").arg(shaderFile.fileName(), shaderFileName);
        return {};
    }
    ...
    if (bytesRead < 0) {
        qCWarning(viewerRendererCategory).noquote() << QStringLiteral("Failed to read shader data '%1' from file '%2': %3").arg(shaderFileName, shaderFile.fileName(), shaderFile.errorString());
        return {};
    }
    ...
    qCInfo(viewerRendererCategory).noquote() << QStringLiteral("Shader data '%1' successfully read from file '%2'").arg(shaderFileName, shaderFile.fileName());
    return shaderData;
}
