...
void sah_kd_tree::Projection<Traits>::findPerfectSplit(
...
{
    {
        ...
        thrust::exclusive_scan_by_key(exec, event.node.cbegin(), event.node.cend(), leftTriangleCountBegin, event.polygonCountLeft.begin());
        ...
        thrust::exclusive_scan_by_key(exec, event.node.crbegin(), event.node.crend(), rightTriangleCountBegin, event.polygonCountRight.rbegin());
    }
    ...
    auto perfectSplitBegin = thrust::make_zip_iterator(layer.splitCost.begin(), layer.splitPos.begin(), layer.polygonCountLeft.begin(), layer.polygonCountRight.begin(), layer.splitEvent.begin());
        ...
        -> PerfectSplitType
    {
        ...
        if (!(min < max)) {
            return {splitCost, splitPos, polygonCountLeft, polygonCountRight, splitEvent};
        }
